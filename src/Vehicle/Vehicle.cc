...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
    ...
    case MAVLINK_MSG_ID_ADC_REPORT:
        _handleAdcReport(message);
        break;
...
void Vehicle::_handleAdcReport(const mavlink_message_t &message){
    mavlink_adc_report_t adcReport;
    mavlink_msg_adc_report_decode(&message, &adcReport);

    float tempVolt = (float)adcReport.channel_value[10];
    int temp = 190;
    for (; temp >= 0; temp--) {
...
void Vehicle::_handleBatteryStatus(mavlink_message_t& message)
{
    ...
    VehicleBatteryFactGroup& batteryFactGroup = bat_status.id == 0 ? _battery1FactGroup : _battery2FactGroup;
     ...
     batteryFactGroup.temperature()->setRawValue(bat_status.temperature);
